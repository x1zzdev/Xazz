...
| `xazz_demo_voiced.mp4` | **음성까지 입힌 제출용 최종본** | — |
| `narration.txt` | 나레이션 평문 — 다른 TTS 에 붙여 넣는 용도 | — |

영상과 자막은 툴체인(Rust·Playwright·ffmpeg) 없이도 바로 볼 수 있도록 한 벌 커밋해 뒀다.
나머지는 `.gitignore` 로 제외돼 있고, 아래 명령이면 언제든 다시 만들어진다.

> 다시 렌더했다고 해서 `xazz_demo.mp4` 를 매번 커밋하지는 말 것 —
> 1MB 바이너리가 이력에 계속 쌓인다. 대본이나 화면이 실제로 바뀌었을 때만 갱신한다.
