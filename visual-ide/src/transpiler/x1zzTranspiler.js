...
export function transpileToX1zz(nodes, edges) {
  ...
  const lines = [];
  lines.push('// ============================================');
  lines.push('// Generated by Xazz Visual IDE Transpiler');
  ...
  // ── model 선언을 항상 최상단에 먼저 배치 ──────────────────────────────────
  // train() 이 모델을 참조하므로, 모델 선언이 파이프라인보다 앞에 와야 한다.
  const processedNodeIds = new Set();
  const modelNodes = validNodes.filter(n => n.type === 'model');
...
let currentPipelineVar = null;
    ...
    chainNodes.forEach(node => {
      ...
      if (result.type === 'source') {
        ...
        result.lines.forEach(l => lines.push(l));
        lines.push('');
      } else if (result.type === 'modelbind') {
        // train → v <modelVar> = <data> |> train(...)
        flushPipeline();
        const dataVar = currentResultVar || currentPipelineVar;
        const modelVar = result.modelVar || 'trained';
        const linesToPush = result.lines.map(l => l.replace('<DATA_VAR>', dataVar));
        const line = linesToPush.join('\n');
        lines.push(line);
        lines.push('');
        lastModelVar = modelVar;
} else if (result.type === 'predict') {
        // predict → 데이터 변수에 예측 컬럼 추가. 데이터 변수 기준 새 statement 시작.
        flushPipeline();
        const dataVar = currentResultVar || currentPipelineVar;
        const predictVar = varName;
        const line = result.lines
          .map(l => l
            .replace('<PREDICT_VAR>', predictVar)
            .replace('<DATA_VAR>', dataVar)
          )
          .join('\n');
        lines.push(line);
        lines.push('');
        // 후속 (take/chart) 노드가 이 예측 변수에 체이닝되도록 설정
        currentPipelineVar = predictVar;
        currentResultVar = null;
      } else if (result.type === 'pipeline') {
        pipelineLines.push('  ' + result.lines.join('\n  '));
  ...
  if (isolatedNodes.length > 0) {
    ...
    isolatedNodes.forEach(node => {
      ...
      if (result.type === 'source' || result.type === 'model') {
        result.lines.forEach(l => lines.push(l));
        lines.push('');
      } else {
