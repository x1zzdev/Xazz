...
- **xazz-compiler**: Lexer → Parser → AST → 정적 타입 검사 → **Typed IR** → 최적화 → Rust Emitter 파이프라인
- **xazz-exec**: Typed IR 을 1회 소비하는 실행 엔진 — `lower`(DataOp→Polars) · `dl`(Burn 텐서) · `dp` · `chart` (무거운 의존성 격리)
- **xazz-runner**: CLI가 서브프로세스로 스폰하는 프로세스 격리 IPC 브리지 (실행 타임아웃 하드닝 — OS 샌드박스 아님)
...
핵심 데이터 흐름: `.xzz` 스크립트 → 파서/AST → 정적 타입·널 안전성 검증 → **Typed IR** → 보안 가드레일 검사 → (위반 시 sLM 자동 보정 + 위반 사유 리포트) → IR lowering → Polars LazyFrame 연산 그래프 → DP 노이즈 주입 → Burn 텐서 변환 → 학습/예측 → 결과 (CSV/차트 HTML/예측) → SHA-256 감사 로그 기록
...
- **[데이터 가속 엔진]** 전처리 명령을 Polars LazyFrame 연산 그래프로 변환·실행 (pandas 대비 228K 행 1.39배, 912K 행 1.95배, 409만 행 1.39배, README Performance 참고)
...
- **연산 효율성**: Apache Arrow 기반 메모리 레이아웃과 Rust 런타임으로 Python 환경 대비 파이프라인 성능·자원 효율을 벤치마크 최대 1.95배(912K 행)까지 향상. 이는 대규모 데이터 전처리 워크로드에서 운영 비용을 직접 절감하는 실무적·엔터프라이즈 가치를 제공한다.
