/// xazz-compiler/src/runtime.rs — 런타임 실행 엔진 (v0.18)
///
...
pub fn run_pipeline(
    ...
    if let Some(ref name) = last_var_name {
        if let Some(df) = symbol_table.get(name) {
            ...
            // ── [xazz:result] JSON 마커 (xazz-server API 가 stdout 에서 파싱) ──
    ...
    for op in ops {
        match op {
            ...
            PipelineOp::Chart(config) => {
                ...
                // [xazz:chart] 마커로 프론트엔드가 파싱 가능하게 출력 (두 줄: 마커 + compact JSON)
                println!("[xazz:chart]");
...
fn check_division_by_zero(df: &polars::frame::DataFrame, expr: &Expr) {
    ...
    for (denominator_col, expr_context) in denominators {
        if let Ok(series) = df.column(&denominator_col) {
            ...
            if zero_count > 0 {
                eprintln!(
                    "[xazz WARN] DivisionByZero: 컬럼 '{}' 에 0이 {} 개 포함되어 있습니다.",
        ...
        Expr::BinOp { lhs, op, rhs } => {
            if *op == BinOpKind::Div {
                ...
                if is_zero_literal(rhs.as_ref()) {
                    let context = format_expr_for_display(expr);
                    eprintln!(
                        "[xazz WARN] DivisionByZero: 0으로 나누기가 감지되었습니다. ({})",
                        context
                    );
                }
            }
...
fn write_chart_html(spec: &ChartSpec, output_path: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Chart.js type 매핑 (xazz타입 → Chart.js 타입)
